    ...
    <Layout style={{ minHeight: "100vh" }}>
      <header className='app-header'>
        ...
        <nav className='app-nav'>
          {navItems.map((item) => (
            <Link key={item.key} to={item.href} className={`app-nav-item${activeKey === item.key ? " active" : ""}`}>
              {item.label}
            </Link>
          ))}
        </nav>
        <button className='app-hamburger' onClick={() => setMenuOpen((o) => !o)} aria-label='菜单'>
      ...
      {menuOpen && (
        <div className='app-mobile-menu'>
          {navItems.map((item) => (
            <Link key={item.key} to={item.href} className={`app-mobile-menu-item${activeKey === item.key ? " active" : ""}`}>
              {item.label}
            </Link>
          ))}
        </div>
      ...
      <Content style={{ flex: 1 }}>
        <Routes>
          ...
          <Route path='/tools/mortgage' element={<MortgageCalcPage />} />
